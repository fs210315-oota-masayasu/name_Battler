package com.example.namebattler.function.enemy
...
import com.example.namebattler.database.enemyDatabase.EnemyList
...
class EnemyParameterProduction {
    ...
    fun getEnemyData() : List<Characters>{
        ...
        val randomSelectJob = JobParameterProduction().jobEnumList.takeAtRandom(3)
        ...
        randomSelectEnemyList.forEach {
            ...
            val enemyData = Player(it,JobParameterProduction().getJobList(extractJobEnum.jobName)).getParam()?:
            Characters(
                "Error:パラメータ取得失敗",
                0,
                0,
